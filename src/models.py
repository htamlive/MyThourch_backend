...
class DocumentInteraction():
    ...
    def extract_topic_from_database(self, context: str) -> list:
        if (REDIS_ACTIVATE == "TRUE"):
            topics = self.model.extract_topic(context)
            filtered_topics = []
            for topic in topics:
                current_related_topic = self.redis.query_topic(topic)
                if(current_related_topic != None):
                    filtered_topics.append(current_related_topic)
